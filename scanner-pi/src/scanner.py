...
def save_as_csv(data: Dict[str, QRCodeData], output_path: str):
    with open(output_path, "w") as f:
        f.write("id, data,type,rect,polygon,quality,orientation\n")
        for key, value in data.items():
            f.write(
                f"{key},{value.data},{value.type},{value.rect.to_dict()},{[point.to_dict() for point in value.polygon]},{value.quality},{value.orientation}\n"
            )


def get_x_y_of_qr(image_path: str) -> Dict[str, QRCodeData]:
